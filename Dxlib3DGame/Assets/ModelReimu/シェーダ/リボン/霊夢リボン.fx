...
float EdgeMorph : CONTROLOBJECT < string name = "(self)"; string item = "エッジの太さ"; >;

//エッジ太さ
static float EdgeSize = 0.0001 + 0.001 * (EdgeMorph * 1);
//エッジ濃さ
...
float4 BufferShadow_PS(BufferShadow_OUTPUT IN, uniform bool useTexture, uniform bool useSphereMap, uniform bool useToon) : COLOR
{
    ...
    */
    ...
    //float test = max(0,dot( normalize(IN.Eye), normalize(IN.Normal) ));
    ...
    if( any( saturate(TransTexCoord) != TransTexCoord ) ) {
        ...
        */
        ...
        if(useToon){
			...
			s = pow(s,5);
			ans.rgb = lerp(sColor,mColor,s);
			ans.rgb += pow(max(0,dot( normalize(IN.Normal), normalize(normalize(-normalize(IN.Eye) ) ))+0.75),2)
			 * Color * 2;
			return saturate(ans);			
        }else{
       		ans = (EgColor + MaterialDiffuse * pow(dot(normalize(IN.Normal), -LightDirection ) *0.5+0.5,1/0.5));
